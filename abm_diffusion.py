...
N_steps = 35_000_000  # number of time steps to integrate
...
@nb.jit(nopython=True)
def sweep_p(N, colloid_x, colloid_y,colloid_s,probe_x,probe_y,probe_vx,probe_vy):
    '''
    Simulate the composite dynamics; calculate the mean squared displacement of the probe.
    Performs N time steps; initial values (7 variables) should be given.
    '''
    probe_x0,probe_y0=probe_x,probe_y
    ...
    for i in range(N):
        if i%save_rate==0:
            ...
            msd[i//save_rate] = (probe_x-probe_x0)**2+(probe_y-probe_y0)**2
        ...
        probe_y = probe_y+(d_probey1+d_probey2)/2
        #不对位置做周期性边界条件
    ...
    return colloid_x, colloid_y, colloid_s,probe_x,probe_y, probe_vx,probe_vy,  msd
...
def calculate(N_steps):
    ...
    msd2 = np.empty((loop_per_cpu,N_steps//save_rate))

    for i in range(loop_per_cpu):

        colloid_x, colloid_y, colloid_s,probe_x,probe_y,probe_vx,probe_vy =sweep_p(N_eq, colloid_x, colloid_y, colloid_s,probe_x,probe_y,probe_vx,probe_vy)[:7]

        msd2[i] =sweep_p(N_steps, colloid_x, colloid_y, colloid_s,probe_x,probe_y,probe_vx,probe_vy)[-1]
...
with h5py.File("ABM_msd_{}_{}_{}.h5".format(int_force,tumble_rate,mass), "w") as opt:
    opt["L"]=L
